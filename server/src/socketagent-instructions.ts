...
export const REMEMBER_TOOL_DESCRIPTION =
  "Search and retrieve this SocketAgent session's complete durable transcript, including context that may have been compacted out of your active model window. Use search first when the user refers to earlier work you cannot reliably recall, then use context or get with the returned stable session_seq/entry_id. Use list for bounded sequence-based paging and runs to locate earlier user prompts and completed runs. Results default to this session. Use search_all to search all SocketAgent transcripts on this server, including cleared-context archives. Each global request requires the user to approve a Transcript access card. Pass a returned source_id to get/context to read a global match, with fresh approval. Never bypass denial by reading transcript files or using another tool. Results are bounded; narrow or paginate searches instead of dumping large portions of history into context.";
