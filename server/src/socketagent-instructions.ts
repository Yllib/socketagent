...
export const WORK_REVIEW_TOOL_DESCRIPTION =
  "Create and manage a durable human review handoff: describe what you did, provide one or more links or other targets where the user can inspect it, and ask for a decision. Use it for any reviewable work such as a live production environment, sandbox/dev server, file, image, diff, session, or custom target. Primary HTTP(S) targets are embedded inside the app beneath the collapsible review panel so the user can inspect the live site and take notes together; do not request external display for a primary web target. For a rich multi-item dossier, create one HtmlPlan, pass its plan ID as linked_html_plan_id, and use html_plan targets whose URI is the matching element anchor. The user reviews every item privately and you receive one consolidated result only when they choose Finish Review. Approval meaning is supplied by the workflow and may explicitly authorize deployment or another follow-up action.";
...
export function buildSocketAgentIntegrationInstructions(options: {
  ...
  const routingRules = [
    ...
    "- Work Review handoff -> WorkReview. Use it to say what you did and provide the target(s) where the user can inspect the result, whether production, development, sandbox, file, image, diff, session, or another kind of link. Primary HTTP(S) targets are embedded beneath the app's collapsible review panel; do not request external display for a primary web target. For a rich multi-item dossier, create one HtmlPlan, pass its plan ID as linked_html_plan_id, and use html_plan targets whose URI is the matching element anchor. The user reviews items and writes notes privately; no draft decisions or notes are exposed to you. You receive exactly one consolidated result only after the user chooses Finish Review. A review's approval meaning is workflow-defined and may explicitly authorize deployment or another action. Use new_round with the same review_id after addressing requested changes.",
