...
### Codex App MCP Tools
Codex sessions get SocketClaude app tools through a per-turn MCP server registered in `src/codex-session.ts`.

- Each Codex turn calls `registerCodexAppMcp(this.createAppToolContext())`, which creates a unique `/codex-mcp/<token>` URL.
- That token maps to one `AppToolContext`; tool handlers use `ctx.getSessionId()` so messages sent to the app include the originating SocketClaude session ID.
- Codex is launched with `-c mcp_servers.socketclaude_app.url="http://127.0.0.1:$PORT/codex-mcp/<token>"`. Use the `socketclaude_app` key; do not quote a hyphenated key.
